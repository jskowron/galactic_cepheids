...
fig, ax = plt.subplots(len(plot_parameters.simulation_ages), 3, figsize=plot_parameters.figsize2)
...
for row, age in enumerate(plot_parameters.simulation_ages):
  ...
  for i in arms_now: # Galaxy NOW
    ...
    marker, color, size = plot_parameters.arm_symbol[i], plot_parameters.arm_colors[i], plot_parameters.arm_symbol_size[i] * 0.6
    for col in [0, 2]:
      ax[row, col].plot(Y, X, '-', color=plot_parameters.arm_colors[i], alpha=plot_parameters.arm_alpha)
      try:
       ax[row, col].plot([Yend], [Xend], ms=size, color=color, marker=marker, mec=color)
      except ValueError:
        ax[row, col].plot([Yend], [Xend], ms=size, color=color, marker='x', mec=color, mew=2.5)

  pos_at_birth = loadtxt('generate_%dMyr.out'%(age), usecols=(0,1))
  pos_now = loadtxt('generate_%dMyr_evolved.out'%(age), usecols=(0,1))

  ax[row, 1].scatter(pos_at_birth.T[1], pos_at_birth.T[0], c=plot_parameters.simulation_colors[row], **plot_parameters.star_marker_params)
  ax[row, 2].scatter(pos_now.T[1],      pos_now.T[0],      c=plot_parameters.simulation_colors[row], **plot_parameters.star_marker_params)

  gal_cepheids = loadtxt('galactic_cepheids_bin%d.dat'%(row + 1), usecols=(0,1))
  gal_cepheids_X, gal_cepheids_Y = project(gal_cepheids.T[1], gal_cepheids.T[0] * 1000.0)   
  ax[row, 0].scatter(gal_cepheids_Y * 0.001, gal_cepheids_X * 0.001, **plot_parameters.gal_cepheids_marker_params)


  ax[row, 0].set_title('Observed cepheids in the %s age bin'%(numerals[row]))
  ax[row, 1].set_title('Simulated birth %.0f Myrs ago'%(age))
  ax[row, 2].set_title('Evolved to the current epoch')
